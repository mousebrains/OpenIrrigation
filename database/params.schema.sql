...
CREATE TABLE params(id INTEGER PRIMARY KEY AUTOINCREMENT, -- id
                    grp TEXT, -- name of group this parameter belongs to
                    name TEXT, -- name of parameter
                    val TEXT, -- value of parameter
                    UNIQUE(grp, name) ON CONFLICT REPLACE -- combination must be unique
                   );
...
CREATE TABLE program(id INTEGER PRIMARY KEY AUTOINCREMENT, -- id
                     ...
                     maxFlow REAL DEFAULT 100, -- max flow target flow
		     UNIQUE (site, name)
                    );	
